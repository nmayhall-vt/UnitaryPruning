...
@everywhere function get_unitary_sequence_1D(;α=.01, k=10)
...
@everywhere function run(generators, parameters, o, ket, nsamples; seed=1)
    Random.seed!(seed)
    rolling_avg = zeros(nsamples)
    rolling_var = zeros(nsamples)
    ...
    rolling_var[1] = samples[1]*samples[1]
    
    for i in 2:nsamples
        rolling_avg[i] = (rolling_avg[i-1]*(i-1) + samples[i])/i
        rolling_var[i] = (rolling_var[i-1]*(i-1) + samples[i]*samples[i])/i
    end
...
for i in 0:16
    ...
    @everywhere generators, parameters = get_unitary_sequence_1D(α=$i*π/32, k=5)
    ...
    #     avg_traji, var_traji = run(generators, parameters, o, ket, nsamples, seed=runi)
    # end
    
    @sync pmap(runi -> run(generators, parameters, o, ket, nsamples, seed=runi), 1:nruns)
    
    avg_traj ./= nruns
    var_traj ./= nruns
    var_traj .= var_traj .- avg_traj.*avg_traj
