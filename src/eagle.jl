...
function eagle_processor(o::PauliBitString{N}; k=10, α=0.01, sequences=[[1, 13], [19, 32], [38, 51], [57, 70], [76, 89], [95, 108], [114, 126]],
    ...
    for ki in 1:k
        ...
        if need_bridges
            for link in bridges
                pi = PauliBitString(N, Z=[link[1], link[2]])
                push!(generators, pi)
                push!(parameters, π/2)

                pi = PauliBitString(N, Z=[link[2], link[3]])
                push!(generators, pi)
                push!(parameters, π/2)
            end
        else #PBC
            pi = PauliBitString(N, Z=[1])
            push!(generators, pi)
            push!(parameters, π/2)
        end
        ## X layer
